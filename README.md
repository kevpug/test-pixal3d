...
> **AMD / Windows users:** this branch adds a ROCm path that needs no compilation —
> see **[README_ROCM_WINDOWS.md](README_ROCM_WINDOWS.md)**. Every CUDA-only dependency
> (`flash_attn`, `flex_gemm`, `cumesh`, `o_voxel`, `nvdiffrast`) has a pure-PyTorch
> fallback that is selected automatically, with VRAM presets down to 6 GB and
> ComfyUI nodes. The CUDA path below is unchanged.
