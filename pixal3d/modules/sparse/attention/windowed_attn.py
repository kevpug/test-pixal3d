...
def _varlen_sdpa(
    ...
    """
    Block-diagonal attention over variable-length sequences using PyTorch SDPA.

    The varlen kernels pack every window into one flat sequence; SDPA has no
    such entry point, so the windows are padded to a common length and the
    padding is masked out. Windows are small and similar in size here, so the
    padding overhead stays close to nothing.
    """
    from ....compat.attention import chunked_sdpa

    device = q.device
    batch = q_seq_lens.shape[0]
    q_max = int(q_seq_lens.max().item())
    kv_max = int(kv_seq_lens.max().item())

    def scatter(x, seq_lens, max_len):
        seq_lens = seq_lens.to(device)
        b_of_row = torch.repeat_interleave(torch.arange(batch, device=device), seq_lens)
        starts = torch.cumsum(seq_lens, dim=0) - seq_lens
        t_of_row = torch.arange(x.shape[0], device=device) - starts[b_of_row]
        padded = x.new_zeros((batch, max_len) + x.shape[1:])
        padded[b_of_row, t_of_row] = x
        return padded, b_of_row, t_of_row

    q_pad, b_of_row, t_of_row = scatter(q, q_seq_lens, q_max)
    k_pad, _, _ = scatter(k, kv_seq_lens, kv_max)
    v_pad, _, _ = scatter(v, kv_seq_lens, kv_max)

    keep = (torch.arange(kv_max, device=device).unsqueeze(0)
            < kv_seq_lens.to(device).unsqueeze(1))            # [B, kv_max]
    out = chunked_sdpa(
        q_pad.permute(0, 2, 1, 3),
        k_pad.permute(0, 2, 1, 3),
        v_pad.permute(0, 2, 1, 3),
        attn_mask=keep.reshape(batch, 1, 1, kv_max),
    ).permute(0, 2, 1, 3)                                      # [B, L, H, C]
    return out[b_of_row, t_of_row]
...
def sparse_windowed_scaled_dot_product_self_attention(
    ...
    if config.ATTN == 'xformers':
        ...
        qf, kf, vf = qkv_feats.unbind(dim=1)                                            # [M, H, C]
        out = _varlen_sdpa(qf, kf, vf, seq_lens, seq_lens)                              # [M, H, C]
...
def sparse_windowed_scaled_dot_product_cross_attention(
    ...
    if config.ATTN == 'xformers':
        ...
        k, v = kv_feats.unbind(dim=1)                                                   # [M, H, C]
        out = _varlen_sdpa(q_feats, k, v, q_seq_lens, kv_seq_lens)                       # [M, H, C]
