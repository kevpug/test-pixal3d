...
class Mesh:
    ...
    def fill_holes(self, max_hole_perimeter=3e-2):
        if not dispatch.cumesh_available():
            # cumesh has no ROCm/Windows build. The GLB export path fills holes
            # again with trimesh, so skipping here loses nothing on that route.
            dispatch.note('Mesh.fill_holes', 'skipped (cumesh unavailable)')
            return
        import cumesh
    ...
    def remove_faces(self, face_mask: torch.Tensor):
        if not dispatch.cumesh_available():
            keep = ~face_mask.to(self.faces.device).bool()
    ...
    def simplify(self, target=1000000, verbose: bool=False, options: dict={}):
        if not dispatch.cumesh_available():
            from ...compat.mesh_ops import simplify_mesh
            new_vertices, new_faces = simplify_mesh(
                self.vertices.detach().cpu().numpy(),
                self.faces.detach().cpu().numpy(),
                target, verbose=verbose,
            )
            self.vertices = torch.from_numpy(new_vertices).to(self.device)
            self.faces = torch.from_numpy(new_faces).to(self.device)
            return
        import cumesh
...
class MeshWithVoxel(Mesh, Voxel):
    ...
    def query_attrs(self, xyz):
        ...
        vertex_attrs = dispatch.grid_sample_3d(
            self.attrs,
